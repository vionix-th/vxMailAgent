...
export default function registerDiagnosticTracesRoutes(app: express.Express, deps: {
  ...
  router.get('/diagnostics/traces', async (req, res) => {
    ...
    const traces = await deps.getTraces(req as any as ReqLike); // canonical insertion order
  ...
  router.get('/diagnostics/trace/:id', async (req, res) => {
    ...
    const t = (await deps.getTraces(req as any as ReqLike)).find(x => x.id === id);
  ...
  router.delete('/diagnostics/trace/:id', async (req, res) => {
    const id = req.params.id;
    const before = await deps.getTraces(req as any as ReqLike);
    const next = before.filter(t => t.id !== id);
    const deleted = before.length - next.length;
    if (deleted > 0) await deps.setTraces(req as any as ReqLike, next);
    res.json({ deleted });
  });
  ...
  router.delete('/diagnostics/traces', async (req, res) => {
    ...
    const deleted = before.length - next.length;
    await deps.setTraces(req as any as ReqLike, next);
    res.json({ deleted });
  });
