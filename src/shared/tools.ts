...
export const TOOL_REGISTRY: ToolSpec[] = [
  ...
  // Workspace (mandatory)
  { name: 'workspace_add_item', category: 'mandatory', description: 'Store content in the director conversation workspace shared by the director and delegated agents. The backend derives provenance; callers provide only display metadata and payload.', parameters: { type: 'object', properties: { label: { type: 'string' }, description: { type: 'string' }, mimeType: { type: 'string' }, encoding: { type: 'string', enum: ['utf8', 'base64', 'binary'] }, data: { type: 'string' }, tags: { type: 'array', items: { type: 'string' } } }, required: ['mimeType','encoding','data'] } },
  { name: 'workspace_list_items', category: 'mandatory', description: 'List all items in the current conversation workspace.', parameters: { type: 'object', properties: {} } },
  ...
  { name: 'workspace_update_item', category: 'mandatory', description: 'Update fields on a workspace item.', parameters: { type: 'object', properties: { id: { type: 'string' }, patch: { type: 'object' } }, required: ['id', 'patch'] } },
  { name: 'workspace_remove_item', category: 'mandatory', description: 'Remove a workspace item.', parameters: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] } },
