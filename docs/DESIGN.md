...
- **Reply Sending**: Not currently exposed via an HTTP endpoint. Provider-integrated sending will use the signature managed at the account level once implemented. There is no global or per-agent/director signature.
